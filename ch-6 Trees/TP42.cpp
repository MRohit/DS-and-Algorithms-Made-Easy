/*
  Problem 42: Check if two Binary trees are isomorphic
  Time Complexity: O(n)
...
bool isIsomorphic (BTree *root, BTree *root2) {
  if (!root && !root2)
    return 1;
  if ((!root && root2) || (root && !root2))
    return 0;
  return (isIsomorphic (root->left, root2->left) && isIsomorphic (root->right, root2->right));
}
int main () {
  BTree *root = NULL;
  ...
  cout<<"\nAre trees isomorphic:"<<isIsomorphic (root, root2);
  cout<<endl;
  return 0;
}
