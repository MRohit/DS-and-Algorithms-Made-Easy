/*
  Problem 22: Finding sum of all nodes in BTree using recursion
  Time Complexity: O(n)
...
int findSumofBTree (BTree *root) {
  if (!root)
    return 0;
  else return (root->data + findSumofBTree (root->left) + findSumofBTree (root->right));
}
int main (void) {
  BTree *root = NULL;
  int path[1000];
  ...
  preOrder (root);
  cout<<"\nSum of all nodes in Tree (using recursion):"<<findSumofBTree (root)<<endl;
